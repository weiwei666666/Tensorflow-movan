...
class LSTMRNN(object):
    def __init__(self,n_steps,input_size,output_size,cell_size,batch_size):
        ...
        with tf.name_scope('train'):
            self.trian_op = tf.train.AdamOptimizer(LR).minimize(self.cost)
    ...
    def add_output_layer(self):
        ...
        l_out_x = tf.reshape(self.cell_outputs,[-1,self.cell_init_state],name='2_2D')
        Ws_out = self.weight_variable([self.cell_init_state.self.output_size])
...
if __name__== '__main__':
    ...
    for i in range(2000):
        ...
        _,cost,state,pred = sess.run(
            [model.trian_op,model.cost,model.cell_final_state,model.pred],
            feed_dict=feed_dict)

        plt.plot(xs[0:],res[0].flattern(),'r',xs[0,:],pred.flatten()[:TIME_STEPS],'b--')
        plt.ylim((-1,2,1.2))
        plt.draw()
        plt.pause(0.3)
